  ...
  <div className=" flex justify-between items-center flex-end p-4 bg-gray-100 rounded-md">
    <div className="flex items-center space-x-4">
      ...
      <div >
        ...
        {currentUserIsAdmin && (
          <div className="flex justify-between items-center p-4 bg-gray-100 rounded-md">
            <span className="text-sm font-medium text-gray-700">Shoot Permission</span>
            <div className="flex items-center gap-4">
              <span className={`text-xs font-medium ${
                shootPermissionEnabled ? 'text-green-600' : 'text-red-500'
              }`}>
                {shootPermissionEnabled ? 'Enabled' : 'Disabled'}
              </span>
              <label className="inline-flex items-center cursor-pointer">
                <input
                  type="checkbox"
                  className="sr-only"
                  checked={shootPermissionEnabled}
                  onChange={(e) => handleShootPermissionToggle(e.target.checked)}
                  disabled={loading || !selectedUserId}
                />
                <div className={`relative inline-block w-12 h-6 rounded-full transition-colors ${
                  shootPermissionEnabled ? 'bg-gray-800' : 'bg-gray-300'
                }`}>
                  <div className={`absolute top-1 left-1 w-4 h-4 bg-white rounded-full transition-transform ${
                    shootPermissionEnabled ? 'translate-x-6' : 'translate-x-0'
                  }`}></div>
                </div>
              </label>
            </div>
